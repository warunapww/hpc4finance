...
int main(int argc, char **argv)
{
	if (argc != 3)
	{
		printf("Usage: %s <path + args> <no. of times>\n", argv[0]);
	//	exit(0);
	}
	argcs = argc;
	argvs = argv;
...
// //			abort();
//
		if (pthread_create(&thread[i], NULL, ThreadF, (void *)i) != 0)
        	{
...
//	WriteSockData(iSock, "/home/cig4/mpich2-install/examples/cpi", strlen("/home/cig4/mpich2-install/examples/cpi"));
	...
	int iAlias = atoi(pzBuf);
		sprintf(pzReqq, "3 -a %d /mnt/models/gbm 100 .5 .15 -m 10 -s 40000", iAlias);
...
//	WriteSockData(iSock, "/mnt/models/gbm -t ibm", strlen("/mnt/models/gbm -t ibm"));
//	WriteSockData(iSock, (char*)sReq.c_str(), sReq.size());
	char *ff = "3 -a 200 /mnt/models/gbm 100 .5 .15 -d 200 -s 10000";
	WriteSockData(iSock, ff, strlen(ff));
//	WriteSockData(iSock, "/home/cig4/mpich2-install/examples/cpi", strlen("/home/cig4/mpich2-install/examples/cpi"));
